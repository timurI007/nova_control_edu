...
class CustomStatus extends Text implements Unfillable
{
    /**
    ...
    /**
     * Specify the values that should be considered "warning".
     *
     * @param  array<int, string>  $failedWords
     * @return $this
     */
    public function warningWhen(array $warningWords)
    {
        return $this->withMeta(['warningWords' => $warningWords]);
    }
    ...
    protected function resolveStatusType()
    {
        ...
        if (in_array($this->value, $this->meta['warningWords'])) {
            return 'failed';
        }
    ...
    protected function resolveTypeClass()
    {
        switch ($this->resolveStatusType()) {
            ...
            case 'failed':
                return 'bg-yellow-100 text-yellow-600 dark:bg-yellow-300 dark:text-yellow-800';
            default:
